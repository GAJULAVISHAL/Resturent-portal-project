...
export const KitchenOrderCard = ({
  ...
  return (
    <div
      className={`bg-white rounded-lg shadow-md p-4 flex flex-col gap-4 border-l-4 ${statusClasses[statusColor].border} ${isDone ? "opacity-70" : ""}`}
    >
      {/* Card Header */}
      <div className="flex justify-between items-center">
        ...
        <div
          className={`text-sm font-semibold ${isDone ? "text-slate-500" : "text-red-600"}`}
        >
        ...
        {order.items.map((item, index) => (
          <div key={index}>
            <div className="flex justify-start items-center">
              <span className="font-bold text-lg text-slate-900 w-8">
                {item.quantity}x
              </span>
              <span className="flex-1">{item.name}</span>
            </div>
            {item.category && (
              <p className="text-slate-500 text-sm ml-8 -mt-1">
                {item.category?.name}
              </p>
            )}
          </div>
